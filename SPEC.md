...
The CLI reports usage, sessions, and estimated equivalent provider list-price cost.
...
If such calls happen to arrive through the override URLs, they are forwarded by the default CAPI route and are not persisted.
...
If VSCode does not send required auth headers to the override URL in practice, token loading can be reconsidered as a future explicit feature.
...
```
copilot-monitor run [--project name] [--port 7733] [--store-bodies]
copilot-monitor configure-vscode
copilot-monitor stats [--since 30d] [--project x] [--endpoint chat]
...
- `--db path/to/store.db` defaults to `$XDG_DATA_HOME/copilot-monitor/store.db`.
- `--json` on read commands emits machine-readable output.
- `--store-bodies` on `run` enables body persistence.
...
`bodies` is only populated when `--store-bodies` is set.
...
- `--store-bodies` stores selected prompt and completion text for debugging and deeper analysis.
...
1. **Skeleton.** `run` starts plain HTTP on `127.0.0.1:7733`, and `configure-vscode` prints the settings snippet.
2. **Path router + transparent forwarding.** Forward `/chat/completions` to `https://api.githubcopilot.com/chat/completions` while preserving incoming auth headers.
3. **Real-time SSE tee capture.** Parse model from request body, parse usage from streamed response, and forward chunks immediately.
4. **SQLite persistence + stats.** Persist captured records and print per-model usage.
5. **Completion and agent paths.** Add `/v1/engines/*`, `/v1/completions`, and `/agents/*` routing and capture.
6. **Estimated cost and sessions.** Add model catalog, cost reports, and 30-minute gap sessionization.
7. **Privacy and polish.** Add `--store-bodies`, `--json`, README quickstart, and test coverage.
