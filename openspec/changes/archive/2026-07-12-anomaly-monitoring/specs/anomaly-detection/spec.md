...
no configured route, including the full path, method, and provider prefix.

#### Scenario: Unknown path recorded

- **WHEN** a POST request arrives at `/v1/new-endpoint` and no route matches
- **THEN** an anomaly is recorded with category `unrouted_path`, severity
  `warn`, path `/v1/new-endpoint`, and method `POST`
...
Copilot CAPI path without an Authorization header.
