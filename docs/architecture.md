...
- `copilot-monitor run` starts a loopback reverse proxy that observes Copilot traffic.
...
1. VS Code or another client sends Copilot traffic to the local proxy started by `internal/cli/run.go`.
...
3. `internal/proxy/forward.go` reads the request body only long enough to extract safe metadata such as `model` and `stream`.
...
5. Responses are streamed back to the client as they arrive.
...
6. `internal/proxy/server.go` persists only metadata and token counts through `internal/store.InsertRequest` when the route is configured to capture.
   WebSocket `/responses` traffic is tunneled by `internal/proxy/websocket.go` and is not persisted.
...
- `CaptureUsage`: persist only the usage tokens found, for chat, agents, messages, and completions.
- `CaptureMetadata`: persist request metadata without requiring usage, currently for embeddings.
- `CaptureNone`, `CaptureTunnel`, `CaptureLocal`: do not persist request rows (model metadata, WebSocket tunneling, local ping).
...
- Add or change a proxied Copilot route: start in `internal/proxy/router.go`, then update the router and proxy tests.
